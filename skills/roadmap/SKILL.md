...
/roadmap --sync-issues            # Skip research, sync .debussy/product/intents.md -> GH Issues
...
| `.debussy/product/intents.md` | Exists -- always eligible for new intents | N/A |
...
Only write `.debussy/product/intents.md`.
...
---
*Generated by `/debussy:roadmap`. Edit `.debussy/product/intents.md` and re-run `--sync-issues` to update.*
BODY
)"
```
...
Issues are derived from `.debussy/product/intents.md` -- the markdown file is the source
...
the issue body will be overwritten. To persist changes, edit `.debussy/product/intents.md`
instead.
...
Run /roadmap --sync-issues to re-sync issues after editing .debussy/product/intents.md.
```
...
1. Read `.debussy/product/intents.md` (required -- if missing, print error and suggest running full `/roadmap` first, then EXIT).
...
- `.debussy/product/intents.md` is the only file with numbered headers (`## NNN -- Name`).
...
| `.debussy/product/intents.md` missing in --sync-issues mode | Print error, suggest running full `/roadmap` first |
