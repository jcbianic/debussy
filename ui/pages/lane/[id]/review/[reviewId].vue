...
interface Review {
  id: string; title: string; source: string; status: 'pending' | 'approved' | 'rejected'; body: string; code: string | null
}

const allReviews: Record<string, Review> = {
  'r-1': {
    id: 'r-1', title: 'Layout structure and sidebar navigation',
    source: 'Unified UI — Implementation Plan · /feedback session', status: 'pending',
  ...
  'r-2': {
    id: 'r-2', title: 'Lane stage/unstage interaction model',
    source: 'Unified UI — Implementation Plan · /feedback session', status: 'pending',
  ...
  'r-4': {
    id: 'r-4', title: 'Nuxt layout structure',
    source: 'PR #42 · layouts/default.vue', status: 'pending',
...
<main class="flex-1 overflow-auto">
  ...
  'r-6': {
    id: 'r-6', title: 'Keyboard navigation shortcuts',
    source: 'Feedback UI Enhancement — Spec · /feedback session', status: 'pending',
  ...
  'r-8': {
    id: 'r-8', title: 'Root cause — port conflict on 3001',
    source: 'Fix: review server startup crash · workflow gate', status: 'pending',
...
const review = computed<Review>(() => allReviews[reviewId] ?? {
  id: reviewId, title: 'Review item', source: 'Unknown', status: 'pending',
  body: 'Review content would appear here.', code: null,
})
