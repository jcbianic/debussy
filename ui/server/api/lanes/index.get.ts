...
export default defineEventHandler(async () => {
  ...
  // Merge lane records with lane data
  ...
  // Add orphaned lane records (branch deleted but record still exists)
  ...
  await Promise.all(
    lanes
      .filter((lane) => lane.checkedOutIn !== null)
      .map(async (lane: Lane) => {
  ...
  try {
    ...
    const rootLane = lanes.find((l) => l.checkedOutIn === 'root') ?? lanes[0]
