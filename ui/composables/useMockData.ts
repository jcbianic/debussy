...
const lanes: Lane[] = [
  {
    id: 'root', branch: 'feat/42-unified-ui', path: '~/debussy', isActive: true,
    groups: [
      {
        id: 'rg-1', title: 'Unified UI — Implementation Plan', icon: 'i-heroicons-document-text', source: '/feedback session', type: 'feedback',
        items: [
          {
            ...
            rounds: [
              { roundNumber: 1, proposedAt: '3h ago', content: 'The layout uses a persistent left sidebar (w-60) with a project header, a lane list, and a separator above the Overview link. The main area fills remaining space and scrolls independently. Dark mode follows system preference.', feedback: 'The sidebar width feels too narrow for lane names that include branch names. Can we bump it to w-72 and add a collapsed state?', feedbackAt: '2h 30m ago', feedbackStatus: 'changes-requested' },
              { roundNumber: 2, proposedAt: '2h ago', content: 'Updated: sidebar is now w-72 with a collapse toggle in the header. Collapsed state shows only icons. Branch names truncate with ellipsis and show a tooltip on hover. Dark mode still follows system preference.' },
            ],
          },
          {
            ...
            rounds: [
              { roundNumber: 1, proposedAt: '2h ago', content: 'The staged lane is marked with a filled blue dot in the sidebar. Non-staged lanes show a faint Stage button on hover. The lane detail page shows a prominent "Stage" or "Push back" button in its header.' },
            ],
          },
          {
            id: 'r-3', title: 'Inbox hierarchy and review groups', subtitle: 'Approved 10m ago', status: 'approved', type: 'feedback', createdAt: '4h ago',
            rounds: [
              { roundNumber: 1, proposedAt: '4h ago', content: 'Reviews are grouped by session or PR. Each group is collapsible. Individual items within a group link to the review detail page.', feedback: 'LGTM — this matches the mental model I had in mind.', feedbackAt: '10m ago', feedbackStatus: 'approved' },
            ],
          },
        ],
      },
      {
        id: 'rg-2', title: 'PR #42 — feat/42-unified-ui', icon: 'i-heroicons-code-bracket', source: 'code review', type: 'code-review',
        items: [
          {
            id: 'r-4', title: 'Nuxt layout structure', subtitle: 'layouts/default.vue', status: 'pending', type: 'code-review', createdAt: '1h ago',
            rounds: [
              { roundNumber: 1, proposedAt: '1h ago', content: 'The layout wraps everything in a full-height flex container. Sidebar is w-60 with flex-col. Main area is flex-1 overflow-auto.', code: `// layouts/default.vue\n<aside class="w-60 flex-shrink-0 flex flex-col ...">\n  <!-- project header, nav, lanes -->\n</aside>\n<main class="flex-1 overflow-auto">\n  <slot />\n</main>`, feedback: 'Use w-72 per the updated layout spec. Also missing aria-label on <aside>.', feedbackAt: '45m ago', feedbackStatus: 'changes-requested' },
              { roundNumber: 2, proposedAt: '30m ago', content: 'Updated sidebar width to w-72, added aria-label="Sidebar navigation", and added role="main" to the main element.', code: `// layouts/default.vue\n<aside aria-label="Sidebar navigation" class="w-72 flex-shrink-0 flex flex-col ...">\n  <!-- project header, nav, lanes -->\n</aside>\n<main role="main" class="flex-1 overflow-auto">\n  <slot />\n</main>` },
            ],
          },
          {
            id: 'r-5', title: 'NavItem component', subtitle: 'components/NavItem.vue', status: 'approved', type: 'code-review', createdAt: '2h ago',
            rounds: [
              { roundNumber: 1, proposedAt: '2h ago', content: 'Simple component wrapping NuxtLink with active state detection and optional badge.', feedback: 'Good. Approved as-is.', feedbackAt: '1h ago', feedbackStatus: 'approved' },
            ],
          },
        ],
      },
  ...
  {
    id: 'wt-feedback', branch: 'feat/feedback-ui', path: '~/debussy/.worktrees/feedback', isActive: false,
    groups: [
      {
        id: 'rg-3', title: 'Feedback UI Enhancement — Spec', icon: 'i-heroicons-document-text', source: '/feedback session', type: 'feedback',
        items: [
          {
            id: 'r-6', title: 'Keyboard navigation shortcuts', subtitle: '⌘K / j·k / Enter', status: 'pending', type: 'feedback', createdAt: '5h ago',
            rounds: [
              { roundNumber: 1, proposedAt: '5h ago', content: 'Proposed shortcuts: j/k to move between items, Enter to open, a to approve, r to request changes, x to reject, ? for help. ⌘K opens a command palette.' },
            ],
          },
          {
            id: 'r-7', title: 'Server startup sequence', subtitle: 'Port detection + auto-open', status: 'pending', type: 'feedback', createdAt: '5h ago',
            rounds: [
              { roundNumber: 1, proposedAt: '5h ago', content: 'On skill invocation: scan for available port starting at 3001, write port to .port file, start Nitro server, open browser to that port. On skill exit: kill server, delete .port file.' },
            ],
          },
        ],
      },
  ...
  {
    id: 'wt-workflow', branch: 'feat/workflow-mon', path: '~/debussy/.worktrees/workflow', isActive: false,
    groups: [],
  },
  {
    id: 'wt-fix', branch: 'fix/review-server', path: '~/debussy/.worktrees/fix', isActive: false,
    groups: [
      {
        id: 'rg-4', title: 'Fix: review server startup crash', icon: 'i-heroicons-bug-ant', source: 'workflow gate', type: 'workflow',
        items: [
          {
            ...
            rounds: [
              { roundNumber: 1, proposedAt: '6h ago', content: 'The review server crashes on startup when port 3001 is already in use from a previous session that was not cleanly terminated. Fix: scan for a free port starting at 3001, use the first available, write it to .port file.' },
            ],
          },
        ],
      },
...
export function useMockData() {
  const pendingCount = (items: ReviewItem[]) => items.filter(i => i.status === 'pending').length

  const lanesWithPending = computed(() =>
    lanes.map(l => ({
      ...l,
      pending: l.groups.flatMap(g => g.items).filter(i => i.status === 'pending').length,
    })),
  )
  ...
  const getCommits = (laneId: string): Commit[] =>
    commitsByLane[laneId] ?? []
